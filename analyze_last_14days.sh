...
# Create a script to analyze PRs and their associated changes for the past 14 days
cat > analyze_prs_last_14days.py << 'EOF'
#!/usr/bin/env python3
"""
...
def main():
    ...
    # Export data to CSV
    csv_data = []
    csv_header = ["PR Number", "Date", "Author", "Merged By", "Title", "Additions", "Deletions", "Total Changes"]
    ...
    author_csv_data = []
    author_csv_header = ["Author", "PRs", "Additions", "Deletions", "Total Changes"]
    ...
    # Write PRs to CSV
    try:
        with open(f"prs_last_14days_{start_date_str}_to_{end_date_str}.csv", 'w') as f:
            f.write(','.join(csv_header) + '\n')
            for row in csv_data:
                f.write(','.join([str(item).replace(',', ';') for item in row]) + '\n')
        print(f"\nPR data exported to prs_last_14days_{start_date_str}_to_{end_date_str}.csv")
    except Exception as e:
        print(f"Error writing PR CSV: {e}")
    
    # Write author stats to CSV
    try:
        with open(f"authors_last_14days_{start_date_str}_to_{end_date_str}.csv", 'w') as f:
            f.write(','.join(author_csv_header) + '\n')
            for row in author_csv_data:
                f.write(','.join([str(item).replace(',', ';') for item in row]) + '\n')
        print(f"Author stats exported to authors_last_14days_{start_date_str}_to_{end_date_str}.csv")
    except Exception as e:
        print(f"Error writing author CSV: {e}")
...
chmod +x analyze_prs_last_14days.py

# Also run the default bitbucket analyzer for comparison
...
# Now run our improved PR analysis script
python analyze_prs_last_14days.py $START_DATE $END_DATE
